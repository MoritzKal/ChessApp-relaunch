<template>
  <DashboardGrid>
    ...
    <div class="is_small"><InfoMetricTile title="Versions" icon="mdi-history" :value="versionsText" /></div>

    <!-- Row 2: Large -->
    <div class="is_large">
      <TableTile title="Schema & Stats" icon="mdi-table" :vm="schemaVm" :loading="schemaLoading">
        <template #tools>
          <v-select v-model="selectedVersion" :items="versionItems" density="compact" hide-details label="Version" style="max-width:160px" />
          <v-menu>
            <template #activator="{ props }">
              <v-btn v-bind="props" size="small" prepend-icon="mdi-export" variant="text">Exportieren</v-btn>
            </template>
            <v-list>
              <v-list-item v-for="fmt in ['parquet','csv','pgn']" :key="fmt" :title="String(fmt).toUpperCase()" @click="onExport(fmt as any)" />
            </v-list>
          </v-menu>
        </template>
      </TableTile>
    ...
    <div class="is_large">
      <TableTile title="Samples" icon="mdi-image-multiple-outline" :vm="sampleVm" :loading="sampleLoading">
        <template #tools>
          <v-btn v-if="nextCursor" size="small" @click="loadMoreSamples">Mehr laden</v-btn>
        </template>
      </TableTile>
    </div>

    <!-- Row 3: Large -->
    <div class="is_large"><ChartTile title="Quality (latest)" icon="mdi-chart-timeline-variant" :vm="qualityVm" :loading="qualityLoading" :range="'24h'" /></div>
...
import { datasetExportUrl } from '@/services/datasets'
...
const versionsText = computed(() => {
  const count = (versionsRef.value?.length ?? 0)
  const latest = (versionsRef.value?.[0]?.version) || (versionsRef.value?.[versionsRef.value.length-1]?.version) || '—'
  return `${count}/${latest}`
})
...
// Quality chart (use single datapoint per metric at now)
const qualityLoading = ref(false)
const qualityVm = ref<SeriesVM | null>(null)
async function loadQuality(){
  ...
  try {
    ...
    qualityVm.value = {
      series: [
        { label: 'Missing', data: [{ x: now, y: norm(q.missingPct) }], color: '#E6A74C' },
        { label: 'Outliers', data: [{ x: now, y: norm(q.outlierPct) }], color: '#5FBF77' },
        { label: 'Duplicates', data: [{ x: now, y: norm(q.duplicatePct) }], color: '#6BB3FF' },
      ]
    }
...
const versionItems = computed(() => (versionsRef.value || []).map((v: any) => v?.version || v?.name || String(v)))
function onExport(fmt: 'parquet'|'csv'|'pgn'){
  const url = datasetExportUrl(id.value, { format: fmt, version: selectedVersion.value || undefined })
