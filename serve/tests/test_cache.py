...
async def test_cache_hits_and_invalidation_on_reload(monkeypatch):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as ac:
        ...
        miss0 = main.CACHE_MISSES.labels(model_id="default", model_version="0")._value.get()
        assert miss0 == 1

        # second call same FEN => hit
        await ac.post("/predict", json={"fen": VALID_FEN})
        hit0 = main.CACHE_HITS.labels(model_id="default", model_version="0")._value.get()
        assert hit0 == 1
        ...
        await ac.post("/predict", json={"fen": VALID_FEN})
        miss1 = main.CACHE_MISSES.labels(model_id="default", model_version="1")._value.get()
        assert miss1 == 1

        # second call after reload => hit for new version
        await ac.post("/predict", json={"fen": VALID_FEN})
        hit1 = main.CACHE_HITS.labels(model_id="default", model_version="1")._value.get()
        assert hit1 == 1
